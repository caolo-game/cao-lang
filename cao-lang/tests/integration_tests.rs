...
fn test_trace_entry() {
    let ir = CaoProgram {
        ...
        lanes: [
            (
                "main".into(),
                Lane::default().with_card(Card::call_function("pooh", vec![])),
            ),
            (
                "pooh".into(),
...
fn test_string_w_utf8() {
    ...
    let program = CaoProgram {
        ...
        lanes: [(
            "main".into(),
            Lane::default().with_card(Card::set_global_var(
                "result",
...
fn simple_if_statement() {
    let program = CaoProgram {
        ...
        lanes: [
            ...
            (
                "pooh".into(),
                Lane::default().with_card(Card::set_global_var("result", Card::ScalarInt(69))),
            ),
        ]
        .into(),
    };
...
fn if_else_test(condition: Card, true_res: Card, false_res: Card, expected_result: Value) {
    let program = CaoProgram {
        ...
        lanes: [
            ...
            (
                "pooh".into(),
                Lane::default().with_cards(vec![Card::set_global_var("result", true_res)]),
            ),
            (
                "tiggers".into(),
                Lane::default().with_cards(vec![Card::set_global_var("result", false_res)]),
            ),
        ]
        .into(),
    };
...
fn nested_module_can_call_self_test() {
    let cu = CaoProgram {
        ...
        submodules: [(
            ...
            Module {
                ...
                lanes: [
                    (
                        "win".into(),
                        Lane::default().with_card(Card::call_function("nie", vec![])),
                    ),
                    (
                        "nie".into(),
                        Lane::default().with_card(Card::set_global_var(
        ...
        lanes: [(
            "main".into(),
            Lane::default().with_card(Card::call_function("winnie.win", vec![])),
        )]
        .into(),
    };
...
fn import_in_submodule_test() {
    ...
    let cu = Module {
        ...
        lanes: [(
            "main".into(),
            Lane::default().with_card(Card::call_function("run", vec![])),
        )]
        .into(),
    };
...
fn can_import_submodule_test() {
    ...
    let cu = Module {
        ...
        lanes: [(
            "main".into(),
            Lane::default().with_card(Card::call_function("winnie.pooh", vec![])),
        )]
        .into(),
    };
...
fn can_import_function_from_super_module_test() {
    let winnie = Module {
        ...
        lanes: [(
            "pooh".into(),
            Lane::default().with_card(Card::call_function("pog", vec![])),
        )]
        .into(),
    };
    let foo = Module {
    ...
    let cu = Module {
        ...
        lanes: [(
            "main".into(),
            Lane::default().with_card(Card::call_function("bar.run", vec![])),
        )]
        .into(),
    };
...
fn import_super_module_test() {
    let winnie = Module {
        ...
        lanes: [(
            "pooh".into(),
            Lane::default().with_card(Card::call_function("bar.pog", vec![])),
        )]
        .into(),
    };
    let foo = Module {
    ...
    */
    let cu = Module {
        ...
        lanes: [
            (
                "main".into(),
                Lane::default().with_card(Card::call_function(
                    "tiggers",
...
fn set_var_to_empty_test() {
    let cu = Module {
        ...
        lanes: [(
            "main".to_string(),
            Lane::default().with_card(Card::set_var("i", Card::Pass)),
        )]
        .into(),
    };
...
fn read_property_shorthand_test() {
    let cu = Module {
        ...
        lanes: [(
            "main".to_string(),
            Lane::default().with_arg("table").with_cards(vec![
